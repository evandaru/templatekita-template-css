...
(function () {
  ...
  function initTabs() {
    ...
    tabContainers.forEach((container) => {
      ...
      if (tabLinks.length > 0) {
        const activeTab = container.querySelector("[data-tab].active") || tabLinks[0];
  ...
  function initThemeSwitcher() {
    ...
    if (!themePicker || !darkModeToggle || !settingsToggle || !settingsMenu) return;
    ...
    document.addEventListener('click', (e) => {
      if (settingsMenu.classList.contains('is-open') && !settingsMenu.contains(e.target) && !settingsToggle.contains(e.target)) {
    ...
    const savedDarkMode = localStorage.getItem('ui-dark-mode') === null ? prefersDark : localStorage.getItem('ui-dark-mode') === 'true';
  ...
  function initInteractiveDataTable() {
    ...
    function displayPage(page) {
      ...
      if (totalRows === 0) {
        ...
        tableInfo.textContent = `Menampilkan ${start + 1} - ${end} dari ${totalRows} entri`;
      }
    ...
    function setupPaginationControls(totalRows) {
      ...
      prevLi.innerHTML = `<a class="page-link" href="#" data-page="${currentPage - 1}" aria-label="Sebelumnya">«</a>`;
      ...
      nextLi.innerHTML = `<a class="page-link" href="#" data-page="${currentPage + 1}" aria-label="Berikutnya">»</a>`;
    ...
    function sortRows(header, shouldUpdateDOM = true) {
      ...
      const isNumeric = !isNaN(parseFloat(currentRows[0]?.cells[index]?.textContent));

      currentRows.sort((a, b) => {
        let aText = a.cells[index].textContent.trim();
        let bText = b.cells[index].textContent.trim();
        let aVal = isNumeric ? parseFloat(aText.replace(/,/g, '')) : aText.toLowerCase();
        let bVal = isNumeric ? parseFloat(bText.replace(/,/g, '')) : bText.toLowerCase();
  ...
  function initCollapsibleSidebar() {
    ...
    const activeSubLink = document.querySelector('.sidebar-collapsible__content a.active');
